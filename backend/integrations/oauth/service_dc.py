"""
OAuth Service (DataConnect Version)
Orchestrates OAuth flows across providers using Firebase Data Connect.
"""
...
from integrations.oauth.protocol import OAuthProvider, OAuthAuthorizationUrl
...
logger = get_logger("OAuthServiceDC")


class OAuthServiceDC:
    """
    High-level OAuth service using DataConnect.

    Coordinates:
    - Provider-specific OAuth handlers
    - Token encryption/decryption
    - State management
    - DataConnect persistence
    """
    ...
    def __init__(
        ...
        state_manager: StateManagerDC,
        providers: dict[str, OAuthProvider],
    ):
        """
        Initialize OAuthServiceDC.

        Args:
            dc: DataConnect client
            config: Application settings
            token_manager: Token encryption manager
            state_manager: OAuth state manager (DataConnect version)
    ...
    async def complete_oauth_flow(
        ...
        try:
            ...
            # Extract Slack team_id if present (for workspace lookup)
            slack_team_id = None
            if integration_type == "slack":
                if not token_response.extra_data or not token_response.extra_data.get("team_id"):
                    raise ValueError(
                        "Slack OAuth response missing team_id - cannot route webhooks"
                    )
                slack_team_id = token_response.extra_data["team_id"]

            # Store in database via DataConnect
    ...
    async def refresh_token(
        ...
        # Update database via DataConnect
        type_str = (
    ...
    async def revoke_integration(
        ...
        # Mark as revoked in database via DataConnect
        type_str = (
            integration_type.value
            if isinstance(integration_type, IntegrationType)
            else integration_type
    ...
    async def _store_integration(
        ...
        """Store or update integration in database via DataConnect."""
        await self.dc.execute_mutation(
    ...
    async def _get_integration(
        ...
        """Get integration from database via DataConnect."""
        if isinstance(integration_type, IntegrationType):
    ...
    def _to_legacy_format(self, integration: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            "workspace_id": integration.get("workspaceId"),
            ...
            "connected_by_user_id": integration.get("connectedByUser", {}).get("id")
            if integration.get("connectedByUser")
            else None,
        }
